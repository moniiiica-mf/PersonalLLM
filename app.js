...
(function () {
  ...
  function localBrain(input) {
    ...
    if (lower.includes("set design") || lower.includes("production design") || lower.includes("art direction") || lower.includes("film project") || lower.includes("dining scene") || lower.includes("stockholm dilemma") || lower.includes("stockholm")) {
      return { text: pick([
        "For Stockholm Dilemma (November 2025), Monica worked as Production Designer — handling set design and costume coordination for a key dining scene, all within a 2-day shoot. The scene centered around a table filled with food, which required careful planning to make sure the composition worked both narratively and visually on camera. She started by photographing the empty table on location with the director to understand camera framing and spatial constraints, then added plates and objects to capture how different elements would sit within the frame.",
        "Monica's production design work on Stockholm Dilemma (November 2025) involved shaping the entire visual environment of a dining scene. With only 2 days for the full set setup and shoot, efficiency was key. A central challenge was composing a table filled with food that looked intentional on camera. She began with spatial reference photography on location — shooting the empty table setup to understand the framing, then layering in objects to test scale, negative space, and how everything would read from the camera's perspective.",
        "In the Stockholm Dilemma project (November 2025), Monica served as Production Designer, focusing on set styling and the visual composition of a key dining scene. The entire production — set setup and shooting — was completed in just 2 days. She worked closely with the director, starting with reference photography of the location to understand spatial constraints and camera framing. From there, she planned the food plating, table arrangement, and overall set styling to ensure everything felt cohesive on screen.",
      ]), gallery: true };
    }
    ...
    if (lower.includes("what did she do") || lower.includes("her role") || lower.includes("what was her role") || lower.includes("what was monica's role")) {
      ...
      if (projectContext) {
        return "Monica's role was Production Designer on Stockholm Dilemma, which was produced in November 2025. She focused on set design and costume coordination for a key dining scene in the film, with the full set setup and shoot completed in just 2 days. That meant shaping the visual environment of the space so that the objects, food, and characters on screen felt intentional and visually cohesive. She worked closely with the director throughout — from spatial reference photography and visual exploration to concept visualization and final set styling.";
      }
    }
    ...
    if (lower.includes("challenge") || lower.includes("difficult") || lower.includes("hardest part") || lower.includes("what was hard")) {
      ...
      if (projectContext3) {
        return "With only 2 days for the entire set setup and shoot, time was a major factor. A key challenge was making the food-filled table work both narratively and visually on camera within that tight window. Monica had to carefully plan the composition — balancing food placement, color, density vs. negative space, and how the table would read from different camera angles including overhead shots. Translating her digital explorations and sketches into the physical set styling while staying true to the visual direction under time pressure was another challenge she navigated.";
      }
    }

    if (lower.includes("when") || lower.includes("how long") || lower.includes("timeline") || lower.includes("how many days") || lower.includes("duration")) {
      var projectContext8 = conversation.some(function(m) {
        return m.role === "assistant" && (m.content.toLowerCase().includes("stockholm dilemma") || m.content.toLowerCase().includes("production designer"));
      });
      if (projectContext8) {
        return "Stockholm Dilemma was produced in November 2025. The entire production — including set setup, styling, and shooting — took just 2 days in total. That tight timeline made the pre-production work (spatial reference photography, style boards, and AI concept visualizations) especially important, since Monica needed to arrive on set with a clear plan already in place.";
      }
    }
    ...
    if (lower.includes("what did she learn") || lower.includes("what she learn") || lower.includes("takeaway") || lower.includes("take away") || lower.includes("gain from")) {
      ...
      if (projectContext4) {
        return "From the Stockholm Dilemma project (November 2025), Monica gained hands-on experience in the full production design pipeline — from spatial planning and visual exploration to concept visualization and physical set styling, all within a 2-day production timeline. She deepened her understanding of how spatial composition impacts storytelling on camera, and strengthened her ability to use both AI tools and traditional sketching to communicate visual ideas during pre-production. It reinforced her passion for creating intentional, cohesive visual environments, even under tight timelines.";
      }
    }
